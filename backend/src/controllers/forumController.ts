...
export class ForumController {
  ...
  static async update(req: Request, res: Response) {
    try {
      ...
      const forum = await ForumService.update(
        id,
        req.user.id,
        req.user.isAdmin,
        title,
  ...
  static async createReply(req: Request, res: Response) {
    try {
      ...
      if (isNaN(forumId)) return res.status(400).json({ message: 'Forum ID tidak valid' });
      if (!content?.trim()) {
  ...
  static async updateReply(req: Request, res: Response) {
    try {
      ...
      if (isNaN(id)) return res.status(400).json({ message: 'ID balasan tidak valid' });
      if (!content?.trim()) {
        return res.status(400).json({ message: 'Konten balasan tidak boleh kosong' });
      }
  ...
  static async deleteReply(req: Request, res: Response) {
    try {
      ...
      await ForumService.deleteReply(id, req.user.id, req.user.isAdmin);
